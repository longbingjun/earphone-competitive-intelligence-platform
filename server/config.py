...
def get_settings() -> Settings:
    load_local_env()
    minio_endpoint = os.getenv("OBJECT_STORAGE_ENDPOINT", "").strip()
    ...
    return Settings(
        root=ROOT,
        database_url=normalize_database_url(os.getenv("PORTFOLIO_DATABASE_URL", "")),
        ...
        minio_port=_int("OBJECT_STORAGE_PORT", 9000),
        minio_use_ssl=_bool("OBJECT_STORAGE_USE_SSL", False),
        minio_access_key=os.getenv("OBJECT_STORAGE_ACCESS_KEY", "").strip(),
        minio_secret_key=os.getenv("OBJECT_STORAGE_SECRET_KEY", "").strip(),
        minio_bucket=os.getenv("OBJECT_STORAGE_BUCKET", "product-images").strip() or "product-images",
