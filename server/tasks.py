...
def run_image_bootstrap(*, trigger: str = "image_bootstrap") -> dict[str, Any]:
    ...
    if not settings.database_configured:
        raise RuntimeError("PORTFOLIO_DATABASE_URL is required for image bootstrap")
...
def run_full_image_backfill(*, trigger: str = "image_full_sync") -> dict[str, Any]:
    ...
    settings = get_settings()
    if not settings.database_configured:
        raise RuntimeError("PORTFOLIO_DATABASE_URL is required for full image synchronization")
...
def run_refresh(*, trigger: str = "schedule", retry_delays: tuple[int, ...] = RETRY_DELAYS_SECONDS) -> dict[str, Any]:
    ...
    settings = get_settings()
    if not settings.database_configured:
        raise RuntimeError("PORTFOLIO_DATABASE_URL is required for the refresh job")
