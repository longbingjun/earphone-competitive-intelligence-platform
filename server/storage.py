...
def minio_connection_target(settings: Settings) -> tuple[str, bool]:
    ...
    if "://" in raw:
        ...
        if parsed.path not in {"", "/"}:
            raise ValueError("OBJECT_STORAGE_ENDPOINT must not include a path")
        raw = parsed.netloc
        secure = parsed.scheme.lower() == "https"
    if not raw:
        raise ValueError("OBJECT_STORAGE_ENDPOINT is required")
    if ":" not in raw:
...
class MinioObjectStorage:
    def __init__(self, settings: Settings):
        if not settings.minio_configured:
            raise RuntimeError("OBJECT_STORAGE_ENDPOINT and MinIO credentials are required")
