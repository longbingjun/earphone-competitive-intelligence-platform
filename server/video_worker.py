...
def acquire_video(snapshot: dict[str, Any], work_dir: Path, settings: Settings) -> AcquiredVideo:
    ...
    session_file = settings.video_session_file.strip()
    if session_file and not Path(session_file).is_file():
        raise FileNotFoundError(f"APP_VIDEO_SESSION_FILE does not exist: {session_file}")
    ...
    if session_file:
        options["cookiefile"] = session_file
